...
import "./BcFormularioModal.css";

export default function BcFormularioModal({
  title,
  subtitle,
  error,
  children,
...
export function BcFormularioModalLinha({ children, className = "" }) {
  const classes = ["bc-form-modal__row", className].filter(Boolean).join(" ");
  return <div className={classes}>{children}</div>;
}

export function BcFormularioModalAreaTexto({
  id,
  label,
  name,
  placeholder,
